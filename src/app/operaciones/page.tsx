...
export default function OperationsPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Cargando historial de operaciones financieras...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center bg-rose-50 dark:bg-[#17111e]/20 border border-rose-200 dark:border-rose-500/20 rounded-xl p-8">
        ...
        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">Error al consultar operaciones</h3>
  ...
  // Filtrar operaciones por nombre de cliente o RFC
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 dark:border-[#1e293b]/40 pb-5">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Historial de Operaciones</h1>
      ...
      <div className="flex justify-end">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Buscar por ID, RFC o Razón Social..."
            value={search}
      ...
      {filteredOperations.length === 0 ? (
        <Card className="bg-slate-50 dark:bg-[#111625]/20 border-slate-200 dark:border-[#1e293b]/40 py-16 text-center">
          <CardContent className="flex flex-col items-center justify-center">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">No se encontraron operaciones registradas</p>
            <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">Haz clic en Nueva Originación para fundear el primer lote de facturas.</p>
          </CardContent>
        </Card>
      ) : (
          ...
          <Table>
            <TableHeader className="bg-slate-50 dark:bg-[#111625] border-b border-slate-200 dark:border-[#1e293b]/40">
              <TableRow className="border-b border-slate-200 dark:border-[#1e293b]/40 hover:bg-transparent">
                ...
                <TableHead className="text-slate-600 dark:text-slate-400 font-semibold py-4">ID Operación</TableHead>
                <TableHead className="text-slate-600 dark:text-slate-400 font-semibold py-4">Cliente</TableHead>
                <TableHead className="text-slate-600 dark:text-slate-400 font-semibold py-4">Monto Total</TableHead>
                <TableHead className="text-slate-600 dark:text-slate-400 font-semibold py-4">Anticipado (85%)</TableHead>
                <TableHead className="text-slate-600 dark:text-slate-400 font-semibold py-4">Comisión (1.5%)</TableHead>
                <TableHead className="text-slate-600 dark:text-slate-400 font-semibold py-4">Monto Depositado</TableHead>
                <TableHead className="text-slate-600 dark:text-slate-400 font-semibold py-4 text-right">Fecha</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredOperations.map((op) => {
                ...
                return (
                  <Fragment key={op.id}>
                    {/* Fila Principal */}
                    <TableRow className="border-b border-slate-100 dark:border-[#1e293b]/20 hover:bg-slate-50 dark:hover:bg-[#1e293b]/10 transition-colors">
                      ...
                      <TableCell className="py-3.5">
                        {client ? (
                          ...
                          <span className="text-slate-500 font-medium">Cliente Desconocido</span>
                        )}
                      ...
                      <TableCell className="text-right text-slate-500 dark:text-slate-400 py-3.5 text-xs">
                    ...
                    {isExpanded && (
                      <TableRow className="bg-slate-50/80 dark:bg-[#0b0f19] border-b border-slate-200 dark:border-[#1e293b]/20 hover:bg-slate-50 dark:hover:bg-[#0b0f19]">
                        <TableCell colSpan={8} className="p-4">
                          <div className="border border-slate-200 dark:border-[#1e293b]/40 rounded-lg overflow-hidden bg-white dark:bg-[#0a0d15] p-4 space-y-3 shadow-xs">
                            <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Facturas Cedidas en este Lote</h4>
                            <Table>
                              <TableHeader className="bg-slate-100 dark:bg-[#111625]/60">
                                <TableRow className="border-b border-slate-200 dark:border-[#1e293b]/20 hover:bg-transparent">
                                  ...
                                  <TableHead className="text-slate-500 font-semibold py-1.5 text-xs">Monto Factura</TableHead>
                              ...
                              <TableBody>
                                {op.invoices?.map((inv) => (
                                  <TableRow key={inv.id} className="border-b border-slate-100 dark:border-[#1e293b]/10 last:border-0 hover:bg-transparent">
                                    ...
                                    <TableCell className="text-slate-500 dark:text-slate-400 text-xs py-2">
                                      {new Date(inv.issueDate).toLocaleDateString('es-MX', {
                                        month: 'short',
                                        day: 'numeric',
                                      })}
                                    </TableCell>
                                    <TableCell className="text-right text-slate-500 dark:text-slate-400 text-xs py-2">
